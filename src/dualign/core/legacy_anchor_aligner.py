...
def _enumerate_merge_combos(
    ...
    def spans_around_baseline(
        baseline: int,
        baselines: Set[int],
        size: int,
    ) -> Iterator[_IndexTuple]:
        """Yield every legal span containing exactly this baseline.

        The previous implementation grew independently to the left or right,
        so a legal block containing free lines on both sides of its baseline
        was silently absent from the final DP.  The nearest neighbouring
        baselines define the admissible interval; the length bound only limits
        how much of that interval is enumerated.
        """

        left_boundary = baseline
        while left_boundary > 0 and left_boundary - 1 not in baselines:
            left_boundary -= 1
        right_boundary = baseline + 1
        while right_boundary < size and right_boundary not in baselines:
            right_boundary += 1

        first_start = max(left_boundary, baseline - theta + 1)
        for start in range(first_start, baseline + 1):
            first_end = max(baseline + 1, start + min_len)
            last_end = min(right_boundary, start + theta)
    ...
    for a_s, a_t, _a_sc in anchors:
        ...
        for source_span in spans_around_baseline(src_idx, B_S, n):
            src_combos.append((source_span, (tgt_idx,)))

        for target_span in spans_around_baseline(tgt_idx, B_T, m):
            tgt_combos.append(((src_idx,), target_span))
...
def align(
    ...
    if n == 0 or m == 0:
        if n and not config.allow_deletions:
            raise ValueError("目标为空且配置禁止原文孤行，无法完整覆盖")
        if m and not config.allow_insertions:
            raise ValueError("原文为空且配置禁止译文孤行，无法完整覆盖")
        ops: List[_AlignmentOperation] = [
            *((((index,), (), 0.0) for index in range(n))),
            *(((), (index,), 0.0) for index in range(m)),
        ]
        return AlignmentResult(
            ...
            sim_matrix=np.empty((n, m), dtype=np.float64),
        )
